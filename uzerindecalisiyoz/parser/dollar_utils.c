...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/20 12:03:29 by ngulcift          #+#    #+#             */
/*   Updated: 2024/09/20 14:29:19 by ngulcift         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*allocate_new_node(const char *node, const char *var, const char *value)
{
	...
	new_node = malloc(ft_strlen(node) + extra_len + 1);
	return (new_node);
}
...
void	switch_node_substr(t_split *node, const char *var, const char *value)
{
	char	*new_node;
	...
	if (!new_node)
		return ;
	switch_substr_in_node(new_node, node->node, var, value);
	free(node->node);
	node->node = new_node;
}
...
void	dollar_and_quest(t_split *split, t_exit_status *exit)
{
	...
	while (current_split)
	{
		...
		while (node[i] != '\0')
		{
			...
			if ((node[i] == '$') && (node[i + 1] == '?'))
			{
				switch_node_substr(current_split, "$?", exit_str);
				exit->exit_code = 127;
			}
			i++;
		}
